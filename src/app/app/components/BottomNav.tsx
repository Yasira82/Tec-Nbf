...
import { Icon, type NbfIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type NbfTab = 'home' | 'templates' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: NbfTab; onSelect: (t: NbfTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: NbfTab; icon: NbfIconName; label: string }[] = [
    { key: 'home',      icon: 'building', label: t.nbf.nav.home      },
    { key: 'templates', icon: 'grid',     label: t.nbf.nav.templates },
    { key: 'pro',       icon: 'star',     label: t.nbf.nav.pro       },
    { key: 'settings',  icon: 'settings', label: t.nbf.nav.settings  },
  ];
  return (
    <nav style={{
